...
`Catalogs` 的本地化日志 / 错误 / 故障文本是随 SDK 版本嵌入的快照。机器人平台更新而 SDK 未更新时，平台上新增的错误码或故障仍能正常上报，只是文本目录中没有对应条目，按原始 ID 回退显示；升级 SDK 即可获得最新文本。用法见 [文本目录 Catalogs](../usage/catalogs.md)。
