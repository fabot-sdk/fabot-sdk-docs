...
本节说明使用 fabot SDK 时贯穿各能力模块的通用机制：如何连接机器人、如何下发命令并跟踪长时操作、如何订阅事件与数据、如何读取状态与故障、如何处理错误与本地化展示文本，以及配置管理、Mock 测试与平台服务管理。
...
6. [文本目录 Catalogs](catalogs.md) — 用嵌入的文本目录把错误、日志、故障格式化成展示文案。
7. [Mock 测试](mock.md) — `Robot.mock()` 的离线开发与测试能力及限制。
8. [配置管理](configuration.md) — `robot.configuration` 的读取与下发、冲突处理与重试。
9. [平台服务管理](services.md) — `robot.services` 对平台服务的启停、配置与状态查询。
...
- **展示本地化文本**：`Catalogs.load()` 后 `format_error` / `format_log` / `format_fault`，见[文本目录 Catalogs](catalogs.md)。
